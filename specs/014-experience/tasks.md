...
- [ ] T001 Bump version to **1.4.0** (`engine/__init__.py`, `packaging/windows.iss`);
  `check_version.py` OK; add `WHATS_NEW["1.4.0"]` stub in `web/main.py`.
- [ ] T002 WS-0 baseline (chrome-devtools-mcp): with the app running, capture a
...
- [ ] T003 Confirm CI is green on `main` post-v1.3.0; if `.github/workflows/ci.yml`
  still fails on Ubuntu, diagnose + fix the Linux-only failures until green.
- [ ] T004 [P] Write failing test `tests/test_web.py::test_startup_bootstrap_runs`
  ...
  Make it green.
- [ ] T005 [P] Quiet warnings: silence the Starlette/`httpx` TestClient +
...
- [ ] T006 [P] Extend `humandate` to every screen: apply `| humandate` in
...
- [ ] T007 Static asset caching + versioning: in `web/main.py` serve
...
- [ ] T008 [US1] Refresh the token layer in `web/static/styles.css` (`:root` +
...
- [ ] T009 [US1] Shared components pass: unify card/table/badge/button/input/
...
- [ ] T011 [P] [US1] Add `tests/test_web.py` assertions: every page returns 200 and
...
- [ ] T012 [US2] **Fix the CLS 0.27 at the source.** The `#update-banner` +
...
- [ ] T013 [US2] Optimistic status in `web/static/app.js`: Save/Applied/Hide flip
  ...
  source of truth.
- [ ] T014 [US2] Motion: adopt the **View Transitions API** for HTMX-boosted page/
...
- [ ] T016 [US3] `web/static/palette.js` (NEW, vanilla): Ctrl/⌘-K opens an ARIA
...
- [ ] T017 [US3] Feed keyboard nav in `app.js`: j/k move focus between job rows,
  Enter opens the focused job, `/` focuses the search/filter — no library.
- [ ] T018 [P] [US3] `tests/test_extension_assets.py`-style static assert (or
