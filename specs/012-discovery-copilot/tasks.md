...
- [ ] T001 Add a one-line Principle-III note to `.specify/memory/constitution.md` (PATCH bump 1.1.2 → 1.1.3 + Sync Impact Report entry): the companion MAY read the current page's public job metadata to render a local-only, READ-ONLY discovery overlay — a bounded read-only addition, not a relaxation of the no-click/no-submit/no-bulk-scrape rules.
- [ ] T002 [P] Create the discovery fixtures directory `tests/fixtures/discovery_pages/` with three pages: `jsonld_jobposting.html` (schema.org JobPosting JSON-LD with title/hiringOrganization.name/description), `linkedin_jobs_view.html` (LinkedIn `/jobs/view` DOM shape, no clean JSON-LD), `indeed_viewjob.html` (Indeed job-page DOM shape). Each also exposes its detected title/company in the DOM so a test can assert extraction.
- [ ] T003 [P] Create empty test modules `tests/test_discovery.py` and `tests/integration/test_discovery_badge.py` with the standard imports/markers (`pytestmark = pytest.mark.browser` for the integration one), ready for red tests.
...
- [ ] T004 [P] Write failing unit test `tests/test_discovery.py::test_get_job_by_url` for a new `db.get_job_by_url(url)` (returns the job dict incl. id for a known url; None otherwise).
- [ ] T005 Implement `engine/db.py::get_job_by_url(url) -> dict | None` (SELECT on the existing `jobs.url UNIQUE`; reuse `_row_to_job`/`get_job` shape or a minimal id+status row). Make T004 green.
...
- [ ] T020 [US2] Add the **Save to Job Engine** button to the badge in `extension/content/discovery.js`: on click, `sendMessage({_je:true, payload:{type:"save_job", url, title, company, description, location}})`; on `save_result` set the button to "Saved ✓"; if `already_saved`/`already` render the "Already saved" state on open.
- [ ] T021 [US2] Extend `tests/integration/test_discovery_badge.py::test_save_persists_and_dedups` (`-m browser`): click Save on a fixture → assert the job appears via the app (DB/query) with correct title/company/url and status `saved`; trigger Save/reload again → assert no duplicate and the badge shows "Already saved".
- [ ] T022 [P] [US2] Add a unit test `tests/test_ext_backend.py::test_save_job_marks_saved_and_dedups` covering the handler-level dedup + status `saved` + `already` flag transitions (fast, non-browser complement to T021).
...
- [ ] T023 [US3] Add collapse (to a minimal re-expandable pill) and per-URL dismiss (✕) controls to the badge in `extension/content/discovery.js`; ensure fixed bottom-right placement with safe margins so it never overlaps page controls; re-detect/re-render on in-place navigation resets the dismissed state for a NEW url only.
- [ ] T024 [US3] Write `tests/integration/test_discovery_badge.py::test_badge_dismiss_collapse_and_readonly` (`-m browser`): dismiss removes the badge; collapse toggles the minimal state; assert (via a page-side sentinel / mutation counter injected by the fixture) that discovery performed zero page mutations/clicks.
- [ ] T025 [US3] Write `tests/integration/test_discovery_badge.py::test_no_interference_with_fill` (`-m browser`): with the discovery badge present, run an existing Apply-Assist-style fill flow on a form fixture and assert fills still complete (reuse an existing fill fixture); assert discovery messages never entered a fill report / changed `bc._state` (unit-level assert acceptable if browser-level is impractical).
...
- [ ] T026 [P] Update `web/main.py` `WHATS_NEW` with a **1.2.0** entry (Discovery badge: browse any job site → match + H-1B sponsorship + one-click Save).
- [ ] T027 [P] Companion page copy: add the discovery-badge blurb to the companion template ("Browse any job site — the badge shows your match + H-1B sponsorship; one click saves it") using existing partials/helpers.
- [ ] T028 [P] Docs: README (new "Discovery badge" capability + read-only/local-only note + reload-extension reminder), `USER_MANUAL` new §, `USER_GUIDE` quick walkthrough. Keep the never-submit/read-only framing consistent with 011.
- [ ] T029 Version bump to **1.2.0** across the gated files (`engine` APP_VERSION, `packaging/windows.iss`, manifest — per `packaging/check_version.py`); run `check_version.py`.
- [ ] T030 Extend `packaging/smoke_test.py` to assert `extension/content/discovery.js` is bundled in the frozen build and the shipped manifest version is `1.2.0`.
- [ ] T031 Verification battery: `pytest -q` ×2, `pytest -q -m browser`, `pytest -q -m slow` all green; fix any regressions before proceeding.
