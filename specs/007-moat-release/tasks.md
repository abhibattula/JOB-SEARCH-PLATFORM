...
- [ ] T027 [US1] TDD: tests/test_routes_autofill.py — extended status payload (queue list with title/company/state, progress, current title+company, fill_report, interrupted, summary), POST /api/autofill/rescan (409 no session), POST /api/autofill/resume-queue (409 nothing to resume); watch fail
- [ ] T028 [US1] web/routes_autofill.py — implement rescan + resume-queue routes and extended status per contracts/http-api.md
- [ ] T029 [US1] web/templates/autofill.html + partials/autofill_status.html — mission-control rebuild: queue list (done ✓ / current ▸ / pending / failed), "N of M" progress, current job title+company, per-field fill report, interrupted banner + Resume button, batch summary view, select-all/none on job list (FR-026)
- [ ] T030 [US1] AUTOFILL_USE_TAILORED_PDF setting (default on) + Settings page toggle; test in tests/test_settings.py
