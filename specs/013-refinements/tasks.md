...
- [ ] T001 Bump version to **1.3.0**: `engine/__init__.py` `APP_VERSION`,
...
- [ ] T002 [P] Create empty test modules `tests/test_default_browser.py` and
...
- [ ] T003 [P] Write failing unit tests `tests/test_default_browser.py` for
...
- [ ] T004 Implement `engine/autofill/default_browser.py`: `default_channel_order(
...
- [ ] T005 [P] Write a failing test for the `humandate` Jinja filter (in
...
- [ ] T006 Register `humandate` in `web/main.py`
...
- [ ] T007 [P] Write failing tests `tests/test_local_llm.py`: `_load_model` passes
...
- [ ] T008 Implement the gpu/threads config + graceful CPU retry in
...
- [ ] T009 [US1] Write failing tests `tests/test_browser_controller.py`:
...
- [ ] T010 [US1] `engine/autofill/browser_controller.py`: replace
...
- [ ] T011 [US1] Surface the active backend + browser in the Apply Assist status
...
- [ ] T012 [US1] Open postings via the OS default handler: in `web/routes_api.py`
...
- [ ] T014 [P] [US2] Persistent sort arrows in `feed_table.html` headers (L23,25):
...
- [ ] T015 [US2] Add a `← Back` control to `job_detail.html` header:
  `history.back()` when `document.referrer && history.length>1`, else `href="/"`;
  add a `.back` style.
- [ ] T016 [P] [US2] Add a static/render assertion (in `tests/test_api.py`) that a
...
- [ ] T017 [US3] Write a failing test (`tests/test_profile_import.py` or
...
- [ ] T018 [US3] Add the resume-text hash guard to `engine/profile_import.py`:
...
- [ ] T019 [US4] Add `packaging/make_icon.py` (Pillow) that renders one source
...
- [ ] T020 [US4] Wire the icon into packaging: `packaging/jobengine.spec` — EXE
...
- [ ] T021 [US4] Add the favicon link to `web/templates/base.html`
  (`<link rel="icon" href="/static/favicon.ico">`); confirm `/static` serves it.
- [ ] T022 [P] [US4] Add an asset test (`tests/test_packaging.py` or extend
...
- [ ] T023 [US4] Extend `packaging/smoke_test.py` to assert the shipped app dir
...
- [ ] T024 [P] Docs — README (browser note: fills in your default browser /
...
- [ ] T025 Fill `WHATS_NEW["1.3.0"]` in `web/main.py` (browser fix, faster AI,
  human dates, sort arrows, Back, icon).
- [ ] T026 Verification battery: `pytest -q` ×2 + `-m browser` + `-m slow` all
