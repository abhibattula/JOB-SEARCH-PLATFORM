...
- [x] CHK004 Is there a verifiable, after-the-fact way (per the requirements, not the implementation) to confirm that no automated submit/login click occurred during a session, supporting SC-003's "0%, no exceptions" claim? [Measurability, Spec §SC-003] — **Resolved** during `/speckit.analyze`: `tasks.md` T022a adds an explicit regression test asserting the browser controller never invokes a click on any submit/login-classified element.
